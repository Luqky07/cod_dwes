...
<body>
	...
	if (!empty($_POST["personas"])) {
		...
		$txt .= "</ul>";
		echo $txt;
		//Asignamos a variables los valores recibidos por el formulario
		$newEmail = $_POST["correo"];
		$newNombre = $_POST["nom"];
		if (empty($newEmail)) { //Si el email esta vacio y se introduce un nombre registrado se mostrarán todos los email con ese nombre
			if (findPos($array, $newNombre) !== false) {
				echo "<p>Prueba de emails</p>";
			}
		} else { //Si el email esta relleno comprobará otros campos
			$busqueda = findPos($array, $newEmail);
			if ($busqueda === false && !empty($newNombre)) {
				$array[$pos++] = $newEmail;
				$array[$pos++] = $newNombre;
			} else {
				if (!empty($newNombre)) {
					$array[$busqueda + 1] = $newNombre;
				} else {
